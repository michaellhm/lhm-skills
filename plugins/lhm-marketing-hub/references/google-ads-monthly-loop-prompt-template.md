...
## Objective
Prepare the monthly Google Ads decision brief for {{CLIENT_NAME}} before Michael starts work: load canonical Obsidian context, gather read-only specialist evidence, save a verified Drive one-pager, create or update the BasicOps review record, and stop waiting for Michael through Hermes. Do not execute proposed actions in this scheduled run.
...
1. **Save and verify the full review pack before any notification or decision handoff.** The Google Ads worker owns these artefacts. Read the exact Google Drive destination from the client's canonical service file and save `monthly-review-YYYY-MM.md`, `conversion-tracking-YYYY-MM.md`, `specialist-findings-YYYY-MM.md`, and `implementation-checklist-YYYY-MM.md` in the same governed `google_ads/YYYY-MM/` folder. The checklist separates approved routine work from `Michael approval required` work and uses atomic exact-object checkboxes. Save every extra specialist execution file in that same folder, link it from the two index files, and read it back as well. Return every observed URL. Do not use an inferred local path or claim completion without readback. If saving or verification fails, return the complete pack content with `needs review` and the exact blocker; do not silently save elsewhere.
