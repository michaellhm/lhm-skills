...
2. **Read-only specialist evidence** — the worker pulls the current 30 days and immediately preceding 30 days; reconciles prior commitments against live settings; runs `bid-budget-optimizer` across every active campaign; runs the monthly tactical `pmax-optimizer` slice whenever PMax is active; runs the read-only/prepare-only `keyword-optimizer` slice whenever Search is active; runs `google-ads-conversion-audit` across account and campaign goal settings; and cross-checks GA4 whenever firing or import state is unclear. Pull AdPulse `pacing` and `kpiPercentage` when available. When it is unavailable, explicitly label the manual pacing/performance calculation as a fallback. These diagnostics and their files are pre-handoff analysis, not live Ads mutations.
...
5. **Resolved decision pack** — build one combined `google-ads-monthly-review-YYYY-MM.md` containing the executive review, conversion, bid/budget, keyword and PMax findings, other triggered evidence, the implementation checklist and a separate `Michael approval required` section. Produce separate implementation files only where directly useful, normally an Editor CSV and negative-keyword TXT when Search evidence supports changes.
6. **Verified delivery and BasicOps handoff** — Hermes dispatches one bounded delivery request to the configured ChatGPT/Codex worker. That worker validates the manifest, saves and reads back the combined review and every promised implementation file in the canonical Drive destination, then creates or updates one BasicOps parent through `lhm-project-hub:basicops-task-manager`. Keep only the governed metadata line and useful URLs in the description. Put key metrics, campaign bid verdicts, the resolved top five, matched-zone optional checklist, authority classes and any exact consequential decision in the discussion. Return every observed URL; do not create execution subtasks merely from report delivery.
...
The delivery request contains the client, run month, combined review, implementation-file manifest, specialist child-run receipts, canonical Drive destination, stable BasicOps dedupe key, overview and resolved actions. The worker must use the installed Google Drive skills and `lhm-project-hub:basicops-task-manager`, then read every file and the task back. A BasicOps handoff is invalid while a required specialist child is non-terminal, QA has not passed, a promised manifest item is absent or any Drive readback is missing.
...
google_ads_monthly_handback:
  ...
  specialist_runs:
    bid_budget: {run_id: "id or null", status: "complete|blocked|not_applicable", qa: "pass|fail|pending"}
    keyword: {run_id: "id or null", status: "complete|blocked|not_applicable", qa: "pass|fail|pending"}
    pmax: {run_id: "id or null", status: "complete|blocked|not_applicable", qa: "pass|fail|pending"}
    conversion: {run_id: "id or null", status: "complete|blocked", qa: "pass|fail|pending"}
  artefacts:
    combined_review_url: "verified URL or null"
