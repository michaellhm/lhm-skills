...
4. **Verified delivery handoff** — the specialist returns the finished one-pager to Hermes. Hermes dispatches one bounded monthly-review delivery request to the configured ChatGPT/Codex worker. That worker saves and reads back the report in the canonical Drive destination, creates or updates the BasicOps review record through `lhm-project-hub:basicops-task-manager`, and returns both observed URLs. Hermes does not need direct Drive or BasicOps connectors.
5. **Lead action selection** — the Google Ads Lead uses the matched zone's action library as candidates, then selects no more than five actions using live evidence, economics, prior commitments and campaign-level exceptions. Each action names evidence, expected outcome, specialist skill, dependencies, authority class and verification method.
6. **BasicOps review record** — the delivery worker creates or updates one parent monthly-review task. Keep only the governed metadata line and useful URLs in its description. Put the overview, top five, authority classes and any exact consequential decision in the discussion. Do not create execution subtasks merely from report delivery. Route to Michael only when at least one exact decision is required; otherwise return to the Google Ads Lead.
...
The delivery request contains the client, run month, canonical report source, canonical Drive destination, stable BasicOps dedupe key, overview and proposed actions. The worker must use the installed Google Drive skills and `lhm-project-hub:basicops-task-manager`, then read both records back.

Only use `status: waiting_michael_hermes` after both `artefact.drive_url` and `basicops.task_url` are observed and verified. Before that, use `status: needs_review`, `waiting_on: delivery_worker`, and describe the exact failed or pending stage. Say **analysis complete; delivery incomplete**, never **review complete**. Preserve the delivery run ID so a later Hermes message resumes it instead of submitting a duplicate.
