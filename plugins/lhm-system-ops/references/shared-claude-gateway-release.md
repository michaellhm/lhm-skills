...
The files named in `shared-claude-gateway-release.json` are the authoritative tracked source for the root-owned shared Claude dispatch path. Release 0.9.25 retains the existing isolated routes and adds the distinct review-only `wordpress-rest` route. It maps to `lhm-wordpress-hub:wordpress-lead` and pins the required skill to `lhm-wordpress-hub:wp-rest-operator`, so Hermes can send existing WordPress and LeadScale REST work through Claude Code CLI without confusing it with Astro or broader WordPress build work. The route remains Skill-only, no-MCP and mutation-free until a separate destination profile is registered. The exact admitted predecessors are dispatcher `b7b1a48d…`, worker `10c681dc…`, dispatch unit `bf840874…`, gateway ACL drop-in `41016cd8…`, and container client `e4bda180…`.
...
Release 0.9.25 is the shared-Claude gateway subrelease packaged inside LHM System Ops 0.9.77.
...
Release 0.9.6 retains the host admission and registry-write protections and governs the authoritative bind-mounted
...
Release 0.9.6 enters Google Ads through the installed `/lhm-marketing-hub:start-googleads` command,
...
Release 0.9.6 grants the dispatcher write access to only the exact handback-registry file. Timestamped
...
Release 0.9.6 also retains the Google Ads reconciliation evidence boundary and explicit Prototype,
...
Release 0.9.6 adds one distinct `seo-lead` specialist contract. It maps the existing
`lhm-marketing-hub:seo` agent and Claude `seo` agent to the exact
`lhm-marketing-hub:start-seo` required Skill under workflow `seo-lead-review`. The existing `seo`
route remains pinned to `lhm-marketing-hub:seo-audit` under `seo-review`, and the
`seo_gsc_readonly` contract and Search Console capability are unchanged. The new route remains
