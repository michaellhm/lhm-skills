...
The supported target for this release is `lhm-project-hub` 0.1.75. Never `git pull` or copy files
...
python3 plugins/lhm-system-ops/scripts/build_project_hub_release.py \
  --output /var/lib/lhm-plugin-releases/lhm-project-hub-0.1.75.zip
```

Record the emitted commit and SHA-256. The builder reads only tracked bytes from `HEAD`, assigns a
fixed ZIP timestamp and mode, and refuses a dirty checkout or any version other than 0.1.75.
...
`8e98009097e50fb76d68ed6a5126d0c0c9a9f6c54f5e49ed9ec298a1c8049e3f`; otherwise stop.
...
{
  "schema_version": 3,
  "approval_id": "project-hub-0.1.75-install",
  "action": "install",
...
```text
/usr/local/libexec/lhm-approved-project-hub-deployer sign project-hub-0.1.75-install
```
...
/usr/local/libexec/lhm-approved-project-hub-deployer install \
  project-hub-0.1.75-install \
  /var/lib/lhm-plugin-releases/lhm-project-hub-0.1.75.zip
```
