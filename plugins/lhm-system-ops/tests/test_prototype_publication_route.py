...
def request(source,request_id='publish-001'):
    ...
    index=next(item for item in manifest if item['path']=='alpha-health/sitemap/index.html')
    return {'schema_version':3,'request_id':request_id,'source_basicops_task':'task-123','governed_parent':'RTB-SITEMAP-PUBLISH-2188809-20260820','destination_profile_id':'prototype-main-v1','operation':'publish_sitemap','credential_reference':'LHM_PROTOTYPE_MAIN_DEPLOY_KEY','client_slug':'alpha-health','project_slug':'sitemap','prototype_kind':'sitemap','repository':'michaellhm/lhm-prototype','branch':'main','expected_base_commit':'a'*40,'source_directory':str(source),'source_package_sha256':dispatch.package_digest(manifest),'source_drive_file_id':'drive-file-123','source_drive_file_sha256':index['sha256'],'source_drive_file_bytes':index['bytes'],'file_manifest':manifest,'qa_evidence_reference':'qa-123','idempotency_key':request_id,'standing_authority_reference':'MICHAEL-PROTOTYPE-PUBLISH-STANDING-20260820','commit_message':'prototype: alpha-health/sitemap approved static package'}
