...
def test_unprivileged_fixture_probe_and_read_cross_only_the_bridge(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(helper.subprocess,'run',lambda argv,**kwargs: calls.append(argv) or SimpleNamespace(returncode=0,stdout=b'transcript'))
    for action in ('probe','read'):
        ...
        assert done.stdout==b'transcript' and calls[-1][0]=='/usr/bin/docker'
