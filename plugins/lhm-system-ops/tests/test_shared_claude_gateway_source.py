...
def test_shared_gateway_sources_match_verified_inventory():
    ...
    assert MANIFEST["release_version"] == "0.9.19"
...
def test_baton_verifies_canonical_text_across_basicops_html_transport():
    ...
    assert "mcp__basicops__request_review" in worker
    assert 'When Required native status is "Under Review", use BasicOps request_review' in worker
...
def test_observer_requires_closed_verification_enum_and_full_timestamp():
    ...
    assert "repair_result['checks'] = [str(name) for name, passed in repair_result['checks'].items() if passed]" in worker
    assert "decoded = html.unescape(message['body'])" in worker
    assert "decoded.startswith('<p>') and decoded.endswith('</p>')" in worker
