...
for f in /usr/local/libexec/lhm-department-snapshot-dispatch /usr/local/libexec/lhm-department-snapshot-broker /usr/local/libexec/lhm-department-result-importer /usr/local/libexec/lhm-seo-envelope-runtime /usr/local/libexec/lhm-workflow-registered-adapter; do
  test -e "$f" || continue
  mv "$f" "$backup/"
done
