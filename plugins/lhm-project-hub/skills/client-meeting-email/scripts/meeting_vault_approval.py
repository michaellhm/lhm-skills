#!/usr/bin/env python3
"""Hermes helper for hash-bound vault approvals."""
...
def status(args):
    if not SAFE_ID.fullmatch(args.approval_id):
        raise SystemExit("invalid approval_id")
    for directory in ("vault-apply-processed", "vault-apply-failed"):
...
def main():
    parser = argparse.ArgumentParser(prog="meeting-vault-approval")
