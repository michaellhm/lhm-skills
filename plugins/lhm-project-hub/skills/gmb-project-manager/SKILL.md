...
description: "Read, create, or update the project-management/gmb.md file for a client's GMB optimisation cycle. Use this when the user mentions 'GMB status', 'where are we at with GMB', 'update GMB project', 'update rankings', 'GMB progress', 'what's left to do', 'mark as done', or wants to check or modify the project tracking document. Also used internally by all agents after completing tasks."
---

# GMB Project Manager

Manages the per-client `project-management/gmb.md` file which tracks the full state of a client's GMB optimisation: current cycle, phase, task completion, focus keywords, and ranking history.
...
Look for `[client_root]/project-management/gmb.md`.

**If it does NOT exist:** Create the `project-management/` directory (if needed) and a new `gmb.md` using the template below. Populate the Overview section from `client_profile.md`. Set Current Phase to "Month 0 — Onboarding". Ask the user for the 7 focus keywords (or suggest them based on the client profile).
...
Write the updated `gmb.md` back to the client's `project-management/` folder. Confirm to the user what was changed.
...
- Updates: `[client_root]/project-management/gmb.md`
- Creates the file and `project-management/` directory if they don't exist
