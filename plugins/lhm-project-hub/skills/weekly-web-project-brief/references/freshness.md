...
Missing canonical records must trigger an evidence search across shared client aliases and known project paths. If the client/project record still cannot be found after successful searches, retain a disclosed project gap using the contract below. Do not invent the record or mark coverage complete. Global access failures remain fatal. Move isolated live-site actions out of the project snapshot without removing them from owner lists. For dated baseline commitments, newer unrelated activity is not proof of completion or cancellation; preserve the commitment until evidence establishes its disposition.


## Disclosed project gaps (normal email)

A genuinely absent project record, unknown approval/date, or unresolved local dependency must not stop verified portfolio updates. Available relevant evidence must still be read. Do not turn access errors, incomplete core-board/Inbox pagination, unread available records, contradictory claims or factual regressions into project gaps. Keep those in material_gaps/review issues and stop sending.

For a source-specific local gap, add research.project_gaps entries `{id, scope: "project", project, source: "gmail"|"obsidian"|"basicops", evidence: [relative evidence file paths]}`. Evidence must be in evidence_files and prove the successful relevant searches, including canonical aliases. For affected snapshot coverage, use `{status: "limited", gap_id: id, evidence: [references]}` instead of complete. Retain at least one complete verified source per snapshot project. Missing records for owner-only one-off actions also belong in project_gaps; they do not create snapshot rows.

Add matching brief.blockers entries `{id, project, issue, impact, owner, next_step}`. All five displayed fields must be specific, nonempty text. Name the person best placed to confirm/fix it; distinguish a recommended follow-up from an assigned task. Business blockers may also use this section without a source gap. The renderer places these near the bottom under **Blockers and information needed**, in HTML and text. Qualify affected row/action claims and dates as unknown/pending confirmation; do not present missing evidence as an established fact or drop pending baseline commitments. A local information gap alone is not proof of stalled delivery.

Independent review may accept these accurately disclosed local gaps while checking all other facts and actions. Review binds brief.json as well as email, research, access and comparison. Separate watchdog alerts remain for runs that cannot produce/send a trustworthy report or cannot confirm delivery.
