...
Explicit recovery: `weekly_runtime.py recover --week YYYY-MM-DD --continue-from YYYY-MM-DD` resumes the original failed research on the current Monday after noon under the current immutable release. It preserves the original failed run, source times and evidence, uses a separate commit-keyed recovery directory, refreshes relevant reads, runs independent review and sends through the same weekly production receipt. It refuses any existing production receipt, which must be reconciled instead of resent. Run recovery in a supervised one-shot service, with no concurrent scheduled worker; never delete status or receipts to force a rerun.
