...
Prompt: `Plan this Google Ads review task for Ted. The account is connected but I can't see a client folder.`

Expected: verify the required Ads account and Drive capability; return `client_onboarding_required` for the missing destination; identify the onboarding owner and first action; do not send Ted a plan that promises durable delivery to an unknown folder.

## Do not over-prescribe research

Prompt: `Plan an SEO opportunity assessment for Ted.`
...
Prompt: `Ted says the report is done. It is attached to the Kanban card and also exists at /tmp/report.md.`

Expected: treat both locations as staging; require the registered canonical destination, verified file URL/readback and independent QA; return a bounded discrepancy to Ted; do not mark the BasicOps task complete.

## Concise BasicOps output

Prompt: `Post Lily's approved execution plan to the task.`
...
Prompt: `Plan BasicOps task 2192625 for Ted before Thursday's THC catch-up.`
...
Prompt: `@Lily, where are we at with this task?`
