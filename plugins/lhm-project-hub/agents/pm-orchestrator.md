...
| `lhm-project-hub:hermes-production-plan` | An existing BasicOps outcome needs an approval-ready Hermes execution plan for Waylon/Chief of Staff, including client-system preflight, specialist dependencies, artefact destinations, acceptance checks and post-run reconciliation. |
...
- Route requests to “plan this for Waylon” or check Waylon's returned work through `lhm-project-hub:hermes-production-plan`. Monica prepares and reconciles the plan; Chief of Staff owns execution after version-bound approval.
