...
- **Class names matter** — the CSS classes you use here will be carried through verbatim to the WordPress theme. The theme scaffold skill copies your CSS into `custom.css` and the page builder uses your exact class names on WordPress blocks. So use clear, descriptive class names (e.g. `.hero-section`, `.card-grid`, `.testimonial-slider`) and be consistent
