"""Render a card-first SEO email; no network access or sending authority."""
import argparse
import html
import json
...
LIGHTS = {'red': ('🔴', 'Red', '#b42318', '#fff1f0'), 'orange': ('🟠', 'Orange', '#a04c00', '#fff5e9'), 'green': ('🟢', 'Green', '#17643b', '#edf8ef'), 'unknown': ('⚪', 'Unverified', '#526070', '#f1f4f6')}
...
def render(data):
    ...
    for c in data['clients']:
        ...
        u = urlsplit(c['task_url'])
        if u.scheme != 'https' or u.netloc != 'app.basicops.com' or u.username or u.password:
...
<table role="presentation" cellspacing="0" cellpadding="0"><tr><td style="background:#18596a;border-radius:6px"><a href="{esc(c['task_url'], quote=True)}" style="display:inline-block;padding:12px 20px;color:#ffffff;text-decoration:none;font-weight:bold">Open review card →</a></td></tr></table>
</td></tr></table></td></tr>''')
        plain += [f'{emoji} {label} | {c["name"]}', c['status_reason'], c['stage'], *['• '+s for s in c['highlights']], 'Start here: '+c['next_action'], 'Open review card: '+c['task_url']]
    if not cards:
